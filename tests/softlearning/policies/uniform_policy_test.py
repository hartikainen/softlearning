...
from softlearning.policies.uniform_policy import (
    ContinuousUniformPolicy, DiscreteUniformPolicy)
...
class DiscreteUniformPolicyTest(tf.test.TestCase):
    def setUp(self):
        self.env = get_environment('gym', 'CartPole', 'v0', {})
        self.policy = DiscreteUniformPolicy(
            input_shapes=self.env.observation_shape,
            output_shape=self.env.action_shape,
            action_range=(0, self.env.action_space.n),
            observation_keys=self.env.observation_keys)
