...
class PointMassSequentialEnv(gym.Env):
    ...
    def step(self, action):
        ...
        action_cost = self.action_cost_coeff * np.linalg.norm(
            action_t_0, ord=2)
        distance_cost = self.distance_cost_coeff * distance_to_goal

        reward = -1.0 * (action_cost + distance_cost)

        goal_done = distance_to_goal < self.goal_threshold
        episode_done = self.current_goal_order.size <= self.goal_counter + 1

        goal_reward = float(goal_done) * self.goal_reward
        reward += goal_reward
